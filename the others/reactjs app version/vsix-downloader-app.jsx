...
export default function App() {
  const [page, setPage] = useState('downloader'); // 'downloader' or 'about'
...
function DownloaderPage() {
  ...
  const [message, setMessage] = useState({ type: '', text: '' }); // 'error', 'success', 'info'
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: e.message || 'Invalid Marketplace URL.' });
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      setMessage({ 
        type: 'error', 
        text: `Error: ${e.message}. This often happens due to browser CORS security rules. Check the console (F12) for details.` 
      });
    } finally {
      setIsLoading(false);
    }
